...
(wget --quiet https://github.com/colonelpanik/mailinabox/blob/main/php-awl-0.62-9.el8.noarch.rpm -O /root/php-awl-0.62-9.el8.noarch.rpm; hide_output dnf --quiet --assumeyes install ./php-awl-0.62-9.el8.noarch.rpm)
...
if [ ! -d /usr/local/lib/owncloud/ ] || [[ ! ${CURRENT_NEXTCLOUD_VER} =~ ^$nextcloud_ver ]]; then
	...
	if [ ! -z ${CURRENT_NEXTCLOUD_VER} ]; then
		...
		if [[ ${CURRENT_NEXTCLOUD_VER} =~ ^14 ]]; then
			...
			hide_output sudo -u nginx php /usr/local/lib/owncloud/console.php app:disable user_external
	...
	sudo -u nginx php /usr/local/lib/owncloud/occ db:add-missing-columns
fi
...
TIMEZONE=$(timedatectl show --value | head -n 1)
