...
if [ ! -f "$STORAGE_ROOT/dns/dnssec/$algo.conf" ]; then
	...
	# anymore anyway.)
	KSK=$(umask 077; cd $STORAGE_ROOT/dns/dnssec; dnssec-keygen -r /dev/urandom -b 2048 -a $algo -f KSK _domain_ );
	...
	ZSK=$(umask 077; cd $STORAGE_ROOT/dns/dnssec; dnssec-keygen -b 1024 -r /dev/urandom -T KEY -a $algo _domain_ );
